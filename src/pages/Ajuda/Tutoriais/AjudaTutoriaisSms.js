...
export default function AjudaTutoriaisSms(){
    ...
    useEffect(() => {
        ...
        async function getTutoriaisEbook(){       
            let list = await api.get('/tutoriais?produto=sms&tipo=ebook');
            setTutoriaisEbook(list.data);
        }
        getTutoriaisEbook();

        async function getTutoriaisVideo(){       
            let list = await api.get('/tutoriais?produto=sms&tipo=video');
                        ...
                        <div className="row no-gutters box-documents">
                            ...
                            <div className="col-12 documents">
                                { tutoriaisEbook.map((row, key) => 
                                    <Link to={row.arquivo} className="bt cl-black">{row.titulo}<img src={IconDownlaod} alt="icone de download" className="icon" /></Link>
                                )}
                            </div>
                        </div>
                    </div>
